...
void AGM_GolfBR::HandleMatchHasStarted() {
	Super::HandleMatchHasStarted();
	BPHandleMatchHasStarted();
}

void AGM_GolfBR::BPHandleMatchHasStarted_Implementation() {

}

void AGM_GolfBR::BeginPlay() {
	Super::BeginPlay();
...
#endif
	GetWorldTimerManager().SetTimer(HandleGameSessionUpdateHandle, this, &AGM_GolfBR::HandleGameSessionUpdate, 1.0f, true, 5.0f);
	GetWorldTimerManager().SetTimer(HandleProcessTerminationHandle, this, &AGM_GolfBR::HandleProcessTermination, 1.0f, true, 5.0f);
...
void AGM_GolfBR::PostSeamlessTravel() {
	Super::PostSeamlessTravel();
	BPPostSeamlessTravel();
}

void AGM_GolfBR::BPPostSeamlessTravel_Implementation() {

}

void AGM_GolfBR::PreLogin(const FString& Options, const FString& Address, const FUniqueNetIdRepl& UniqueId, FString& ErrorMessage) {
...
void AGM_GolfBR::Logout(AController* Exiting) {
#if WITH_GAMELIFT
	if (Exiting != nullptr) {
...
FString AGM_GolfBR::InitNewPlayer(APlayerController* NewPlayerController, const FUniqueNetIdRepl& UniqueId, const FString& Options, const FString& Portal) {
...
void AGM_GolfBR::CountDownUntilGameOver() {
	if (GameState != nullptr) {
		AGolfBRGameState* GolfBRGameState = Cast<AGolfBRGameState>(GameState);
		if (GolfBRGameState != nullptr) {
...
void AGM_GolfBR::PickAWinningTeam() {
	GetWorldTimerManager().ClearTimer(CountDownUntilGameOverHandle);
		...
		if (GolfBRGameState != nullptr) {
			...
			if (GameSessionIdOutcome.IsSuccess()) {
				...
				if (FJsonSerializer::Serialize(RequestObj.ToSharedRef(), Writer)) {
					...
					RecordMatchResultRequest->OnProcessRequestComplete().BindUObject(this, &AGM_GolfBR::OnRecordMatchResultResponseReceived);
					...
					GetWorldTimerManager().SetTimer(EndGameHandle, this, &AGM_GolfBR::EndGame, 1.0f, false, 5.0f);
				}
			}
			else {
				GetWorldTimerManager().SetTimer(EndGameHandle, this, &AGM_GolfBR::EndGame, 1.0f, false, 5.0f);
...
void AGM_GolfBR::HandleProcessTermination() {
	if (ProcessTerminateState.Status) {
		...
		GetWorldTimerManager().SetTimer(EndGameHandle, this, &AGM_GolfBR::EndGame, 1.0f, false, 10.0f);
	}
}

void AGM_GolfBR::HandleGameSessionUpdate() {
	if (!GameSessionActivated) {
		if (StartGameSessionState.Status) {
			GameSessionActivated = true;

			GetWorldTimerManager().SetTimer(PickAWinningTeamHandle, this, &AGM_GolfBR::PickAWinningTeam, 1.0f, false, (float)RemainingGameTime);
			GetWorldTimerManager().SetTimer(CountDownUntilGameOverHandle, this, &AGM_GolfBR::CountDownUntilGameOver, 1.0f, true, 0.0f);
		}
	}
}

void AGM_GolfBR::OnRecordMatchResultResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	GetWorldTimerManager().SetTimer(EndGameHandle, this, &AGM_GolfBR::EndGame, 1.0f, false, 5.0f);
}
