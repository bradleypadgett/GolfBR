...
void UMainMenuWidget::NativeConstruct() {
	...
	if (GameInstance != nullptr) {
		...
		if (GolfBRGameInstance != nullptr) {
			AccessToken = GolfBRGameInstance->AccessToken;
		}
	}
			...
			if (QueryParameters.Split("=", &ParameterName, &ParameterValue)) {
				if (ParameterName.Equals("code")) {
					...
					if (FJsonSerializer::Serialize(RequestObj.ToSharedRef(), Writer)) {
						...
						ExchangeCodeForTokensRequest->OnProcessRequestComplete().BindUObject(this, &UMainMenuWidget::OnExchangeCodeForTokensResponseReceieved);
...
void UMainMenuWidget::OnExchangeCodeForTokensResponseReceieved(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	if (bWasSuccessful) {
			...
			if (JsonObject->HasField("access_token") && JsonObject->HasField("id_token") && JsonObject->HasField("refresh_token")) {
				...
				if (GameInstance != nullptr) {
					...
					if (GolfBRGameInstance != nullptr) {
						...
						GolfBRGameInstance->SetCognitoTokens(AccessToken, IdToken, RefreshToken);
...
void UMainMenuWidget::OnMatchmakingButtonClicked() {
	...
	if (GameInstance != nullptr) {
		...
		if (GolfBRGameInstance != nullptr) {
			AccessToken = GolfBRGameInstance->AccessToken;
			MatchmakingTicketId = GolfBRGameInstance->MatchmakingTicketId;
		}
	}
	...
	if (bSearchingForGame) {
		...
		if (!AccessToken.IsEmpty() && !MatchmakingTicketId.IsEmpty()) {
			TSharedPtr<FJsonObject> RequestObj = MakeShareable(new FJsonObject);
			...
			if (FJsonSerializer::Serialize(RequestObj.ToSharedRef(), Writer)) {
				...
				ButtonTextBlock->SetText(FText::FromString("Find Match"));
		...
		if (FJsonSerializer::Deserialize(Reader, JsonObject)) {
			if (JsonObject->HasField("ticketId")) {
				...
				if (GameInstance != nullptr) {
					UGolfBRGameInstance* GolfBRGameInstance = Cast<UGolfBRGameInstance>(GameInstance);
					if (GolfBRGameInstance != nullptr) {
						GolfBRGameInstance->MatchmakingTicketId = MatchmakingTicketId;
...
void UMainMenuWidget::OnStopMatchmakingResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	...
	if (GameInstance != nullptr) {
		UGolfBRGameInstance* GolfBRGameInstance = Cast<UGolfBRGameInstance>(GameInstance);
		if (GolfBRGameInstance != nullptr) {
			GolfBRGameInstance->MatchmakingTicketId = "";
		}
	}
	UTextBlock* ButtonTextBlock = (UTextBlock*)MatchmakingButton->GetChildAt(0);
	ButtonTextBlock->SetText(FText::FromString("Find Match"));
...
void UMainMenuWidget::PollMatchmaking() {
	...
	if (!AccessToken.IsEmpty() && !MatchmakingTicketId.IsEmpty() && bSearchingForGame) {
		TSharedPtr<FJsonObject> RequestObj = MakeShareable(new FJsonObject);
		RequestObj->SetStringField("ticketId", MatchmakingTicketId);
...
void UMainMenuWidget::OnPollMatchmakingResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	if (bWasSuccessful && bSearchingForGame) {
		TSharedPtr<FJsonObject> JsonObject;
		TSharedRef<TJsonReader<>> Reader = TJsonReaderFactory<>::Create(Response->GetContentAsString());
		if (FJsonSerializer::Deserialize(Reader, JsonObject)) {
